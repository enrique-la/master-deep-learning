...
# <figure>
# <center>
# <img src='http://drive.google.com/uc?export=view&id=1Enc4aR4X-bYMZJWcQVVEb1PU5rjm0Z30' />
# <figcaption>Image zero</figcaption>
# </center>
# </figure>
# 
# 
...
imagePath = DATA_PATH + "number_zero.jpg"
...
# Indexing from 0!!
# 
# As mentioned earlier, since matrices are numpy arrays, the first index will be the `row number` and second index is `column number` (like in Matlab). However, OpenCV has `(x,y)` or `(column,row)` coordinates and not `(row,column)`.
# 
# For example, to access the element at `4th row` and `3th column`, we should use `img[4,3]` in numpy. But as we will see in Image annotation section, we will deal with points in OpenCV functions which are represented as `(x,y)` and thus, the coordinates will be `(3,4)`.
...
# 
# Remember that in Python, ranges go until n-1.
# 
...
# <img src="http://drive.google.com/uc?export=view&id=1GcHlc05Tfq1AREZqwH8skp1wblb1IJwG" width=400>
# 
# 
...
# * reverse the order of channels - **`plt.imshow(img[:,:,::-1])`** swaps the 1st and 3rd channel. It's based on the slice operator iterable[start:stop:step]
